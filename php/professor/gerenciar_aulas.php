...
// As funções para gerenciamento de conteúdo foram removidas conforme solicitado,
// pois o campo de associação foi retirado do formulário.
    ...
    if (empty($titulo_aula) || empty($data_aula) || empty($horario) || empty($turma_id)) {
        ...
        try {
            if ($acao === 'adicionar') {
                ...
                // Associações de conteúdo removidas
...
$sql_aulas = "
    ...
    WHERE 
        ...
        -- OCULTA aulas passadas: Garante que a data e hora combinadas são >= ao momento atual do servidor.
        AND CONCAT(a.data_aula, ' ', a.horario) >= NOW()
    ORDER BY 
        -- Ordena pela data mais próxima (ASC) para que as próximas fiquem no topo.
...
if (isset($_GET['editar'])) {
    ...
    if ($aula_para_editar) {
        // Lógica de conteúdo associado removida
...
<head>
    ...
    <style>
        ...
        /* Estilo para o novo botão de ver detalhes (ícone de olho) */
        .btn-info-detalhes {
            background-color: #099410ff; /* Cor azul-clara (info) */
        ...
        <div class="modal-content">
            ...
            <form action="gerenciar_aulas.php" method="POST" id="formAula">
                <div class="modal-body">
                    ...
                    <!-- CAMPO DE CONTEÚDOS REMOVIDO CONFORME SOLICITADO -->
...
$(document).ready(function() {
    // Inicialização do Select2 removida
